#! python3
# -*- coding: utf-8 -*-
"""Naukri Daily update - Runs randomly on Chrome/Edge in Incognito/InPrivate"""
...
# If Headless = True, script runs in headless mode without visible GUI
headless = False
...
def LoadNaukri(headless):
    """Open Browser (Randomly Chrome or Edge) in Private mode to load Naukri.com"""
    
    # Randomly select between Chrome and Edge
    target_browser = choice(["chrome", "edge"])
    driver = None
    
    log_msg(f"Launching {target_browser.upper()}...")
    ...
    log_msg(f"{target_browser.upper()} Launched (Private Mode)!")
...
def naukriLogin(headless=False):
    """Open Browser and Login to Naukri.com"""
    status = False
    driver = None
...
def run_naukri_update():
    """Main function to run Naukri update - called by scheduler"""
    log_msg("-----Naukri.py Script Run Begin-----")
    driver = None
    try:
        status, driver = naukriLogin(headless)
        if status:
            # Random delay between 3-10 minutes after successful login
            delay = uniform(180, 600)
            log_msg(f"Waiting for {delay:.2f} seconds (random delay between 3-10 minutes)...")
            time.sleep(delay)
...
def main():
    """Main scheduler function - runs the update every 15 minutes"""
    log_msg("=== Naukri Auto-Update Scheduler Started ===")
    log_msg("Script will run every 15 minutes")
    log_msg("Press Ctrl+C to stop the scheduler")
    
    # Run immediately on start
    log_msg("Running initial update...")
    run_naukri_update()
    
    # Schedule to run every 15 minutes
    schedule.every(15).minutes.do(run_naukri_update)
    ...
    while True:
        try:
            ...
            log_msg("=== Scheduler stopped by user ===")
            break
        except Exception as e:
            log_msg(f"Scheduler error: {e}")
